...
(function () {
  ...
  const processedDraftCards = new WeakSet(); // Track which cards have buttons already
  ...
  const selectAllDrafts = () => {
    ...
    const filtered = Array.from(containers).filter(el => {
      return el.querySelector('video, img, [class*="video"], [class*="image"]');
    });
      ...
      downloadBtn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          try {
            ...
            if (urlExt && ['mp4', 'mov', 'webm', 'avi'].includes(urlExt.toLowerCase())) {
              extension = `.${urlExt}`;
            } else {
...
async function renderAnalyzeTable(force = false) {
      ...
      try {
        ...
        if (DRAFTS_RE.test(url)) {
          res.clone().json().then(processDraftsJson).catch((err) => {
            dlog('drafts', 'Error parsing drafts fetch response:', err);
          });
  ...
  function processDraftsJson(json) {
    ...
    for (const item of items) {
      try {
        const draftId = item?.id || item?.generation_id || item?.draft_id;
        if (!draftId) continue;
        ...
        if (typeof nFrames === 'number' && nFrames > 0) {
          ...
          const usedFps = (typeof fps === 'number' && fps > 0) ? fps : SORA_DEFAULT_FPS;
          idToDuration.set(draftId, nFrames / usedFps);
          if (usedFps === SORA_DEFAULT_FPS && DEBUG.drafts) {
